...
func (s *CmdExecutor) VolumeCreate(host string,
	...
	cmd := fmt.Sprintf("%v --mode=script volume create %v ", s.glusterCommand(), volume.Name)
	...
	commands = append(commands, fmt.Sprintf("%v --mode=script volume start %v", s.glusterCommand(), volume.Name))
...
func (s *CmdExecutor) VolumeExpand(host string,
	...
	if s.RemoteExecutor.RebalanceOnExpansion() {
		commands = []string{fmt.Sprintf("%v --mode=script volume rebalance %v start", s.glusterCommand(), volume.Name)}
		err := rex.AnyError(s.RemoteExecutor.ExecCommands(host, commands, 10))
		if err != nil {
...
func (s *CmdExecutor) VolumeDestroy(host string, volume string) error {
	...
	commands := []string{
		fmt.Sprintf("%v --mode=script volume stop %v force", s.glusterCommand(), volume),
	}
	...
	commands = []string{
		fmt.Sprintf("%v --mode=script volume delete %v", s.glusterCommand(), volume),
	}
...
func (s *CmdExecutor) createVolumeOptionsCommand(volume *executors.VolumeRequest) []string {
	...
	for _, volOption := range volume.GlusterVolumeOptions {
		if volOption != "" {
			cmd = fmt.Sprintf("%v --mode=script volume set %v %v", s.glusterCommand(), volume.Name, volOption)
...
func (s *CmdExecutor) createAddBrickCommands(volume *executors.VolumeRequest,
	...
	for index, brick := range volume.Bricks[start:] {
		if index%(inSet*maxPerSet) == 0 {
			...
			cmd = fmt.Sprintf("%v --mode=script volume add-brick %v ", s.glusterCommand(), volume.Name)
		}
...
func (s *CmdExecutor) checkForSnapshots(host, volume string) error {
	...
	commands := []string{
		fmt.Sprintf("%v --mode=script snapshot list %v --xml", s.glusterCommand(), volume),
	}
...
func (s *CmdExecutor) VolumeInfo(host string, volume string) (*executors.Volume, error) {
	...
	command := []string{
		fmt.Sprintf("%v --mode=script volume info %v --xml", s.glusterCommand(), volume),
	}
...
func (s *CmdExecutor) VolumeReplaceBrick(host string, volume string, oldBrick *executors.BrickInfo, newBrick *executors.BrickInfo) error {
	...
	// Replace the brick
	command := []string{
		fmt.Sprintf("%v --mode=script volume replace-brick %v %v:%v %v:%v commit force", s.glusterCommand(), volume, oldBrick.Host, oldBrick.Path, newBrick.Host, newBrick.Path),
	}
...
func (s *CmdExecutor) VolumeSnapshot(host string, vsr *executors.VolumeSnapshotRequest) (*executors.Snapshot, error) {
	...
	command := []string{
		fmt.Sprintf("%v --mode=script --xml snapshot create %v %v no-timestamp", s.glusterCommand(), vsr.Snapshot, vsr.Volume),
...
func (s *CmdExecutor) HealInfo(host string, volume string) (*executors.HealInfo, error) {
	...
	command := []string{
		fmt.Sprintf("%v --mode=script volume heal %v info --xml", s.glusterCommand(), volume),
	}
