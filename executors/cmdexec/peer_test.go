...
func TestSshExecPeerProbe(t *testing.T) {
	...
	f.FakeConnectAndExec = func(host string,
		...
		tests.Assert(t, commands[0] == "gluster --timeout=42 peer probe newnode", commands)
	...
	f.FakeConnectAndExec = func(host string,
		...
		case 1:
			tests.Assert(t, host == "host:22", host)
			tests.Assert(t, len(commands) == 1)
			tests.Assert(t, commands[0] == "gluster --timeout=42 --mode=script snapshot config snap-max-hard-limit 14", commands)
