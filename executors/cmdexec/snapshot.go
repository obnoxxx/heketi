...
func (s *CmdExecutor) snapshotActivate(host string, snapshot string) error {
	...
	command := []string{
		fmt.Sprintf("%v --mode=script --xml snapshot activate %v", s.glusterCommand(), snapshot),
	}
...
func (s *CmdExecutor) snapshotDeactivate(host string, snapshot string) error {
	...
	command := []string{
		fmt.Sprintf("%v --mode=script --xml snapshot deactivate %v", s.glusterCommand(), snapshot),
	}
...
func (s *CmdExecutor) SnapshotCloneVolume(host string, vcr *executors.SnapshotCloneRequest) (*executors.Volume, error) {
	...
	command := []string{
		fmt.Sprintf("%v --mode=script --xml snapshot clone %v %v", s.glusterCommand(), vcr.Volume, vcr.Snapshot),
	}
	...
	command = []string{
		fmt.Sprintf("%v --mode=script --xml volume start %v", s.glusterCommand(), vcr.Volume),
	}
...
func (s *CmdExecutor) SnapshotDestroy(host string, snapshot string) error {
	...
	command := []string{
		fmt.Sprintf("%v --mode=script --xml snapshot delete %v", s.glusterCommand(), snapshot),
	}
